  ...
  public class DisposableManager : IDisposable
  {
      ...
      [Inject(Id = "Late", Optional = true, Source = InjectSources.Local)] List<ValuePair<Type, int>> latePriorities)
    {
      foreach (IDisposable disposable in disposables)
      {
        ...
        int? match = priorities.Where(x => disposable.GetType().DerivesFromOrEqual(x.First)).Select(x => (int?)x.Second)
          .SingleOrDefault();
      ...
      {
        int? match = latePriorities.Where(x => lateDisposable.GetType().DerivesFromOrEqual(x.First))
