...
namespace Zenject
{
  public class PoolableManager
  {
    ...
    {
      int? match = priorities.Where(x => poolable.GetType().DerivesFromOrEqual(x.First)).Select(x => (int?)x.Second)
        .SingleOrDefault();
  ...
  public class PoolableManager<T1, T2, T3, T4>
  {
    ...
    private PoolableInfo CreatePoolableInfo(IPoolable<T1, T2, T3, T4> poolable, List<ValuePair<Type, int>> priorities)
    {
      int? match = priorities.Where(x => poolable.GetType().DerivesFromOrEqual(x.First)).Select(x => (int?)x.Second)
        .SingleOrDefault();
      int priority = match.HasValue ? match.Value : 0;
