...
namespace Zenject
{
  ...
  {
    [Inject(Optional = true, Source = InjectSources.Local)] private readonly List<ITickable> _tickables;

    [Inject(Optional = true, Source = InjectSources.Local)] private readonly List<IFixedTickable> _fixedTickables;

    [Inject(Optional = true, Source = InjectSources.Local)] private readonly List<ILateTickable> _lateTickables;

    [Inject(Optional = true, Source = InjectSources.Local)] private readonly List<ValuePair<Type, int>> _priorities;
    ...
    private readonly TickablesTaskUpdater _updater = new();
    private readonly FixedTickablesTaskUpdater _fixedUpdater = new();
    private readonly LateTickablesTaskUpdater _lateUpdater = new();
    ...
    {
      foreach (Type type in _fixedPriorities.Select(x => x.First))
      ...
      foreach (IFixedTickable tickable in _fixedTickables)
      {
        ...
        List<int> matches = _fixedPriorities.Where(x => tickable.GetType().DerivesFromOrEqual(x.First))
    ...
    {
      foreach (Type type in _priorities.Select(x => x.First))
      ...
      foreach (ITickable tickable in _tickables)
      {
        // Note that we use zero for unspecified priority
        // This is nice because you can use negative or positive for before/after unspecified
        List<int> matches = _priorities.Where(x => tickable.GetType().DerivesFromOrEqual(x.First)).Select(x => x.Second)
          .ToList();
        int priority = matches.IsEmpty() ? 0 : matches.Distinct().Single();
    ...
    {
      foreach (Type type in _latePriorities.Select(x => x.First))
      ...
      foreach (ILateTickable tickable in _lateTickables)
      {
        // Note that we use zero for unspecified priority
        // This is nice because you can use negative or positive for before/after unspecified
        List<int> matches = _latePriorities.Where(x => tickable.GetType().DerivesFromOrEqual(x.First))
          .Select(x => x.Second).ToList();
        int priority = matches.IsEmpty() ? 0 : matches.Distinct().Single();
