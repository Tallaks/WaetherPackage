...
namespace Zenject.Internal
{
  public static class ReflectionTypeAnalyzer
  {
    ...
    private static ReflectionTypeInfo.InjectParameterInfo CreateInjectableInfoForParam(
      Type parentType, ParameterInfo paramInfo)
    {
      ...
      bool isOptionalWithADefaultValue =
        (paramInfo.Attributes & ParameterAttributes.HasDefault) == ParameterAttributes.HasDefault;
    ...
    private static ConstructorInfo TryGetInjectConstructor(Type type)
    {
      ...
      {
        ConstructorInfo explicitConstructor =
          (from c in constructors where _injectAttributeTypes.Any(a => c.HasAttribute(a)) select c).SingleOrDefault();
